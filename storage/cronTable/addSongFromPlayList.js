...
async function addSong(bot, musicTag, playlists) {
    bot.dataBase.get("connection").exec('SELECT 1 FROM ?? WHERE `tagName` = ?', ["musicsList", musicTag], (error, results, fields) => {
        if (error) throw error;

        if (results.length === 0) {
            bot.dataBase.get("connection").exec('INSERT INTO ?? (`tagName`) VALUE (?)', ["musicsList", musicTag], (error, results, fields) => {
                if (error) throw error;

                bot.dataBase.get("connection").exec('SELECT `id` FROM ?? WHERE `tagName` = ?', ["musicsList", musicTag], (error, results, fields) => {
                    if (error) throw error;

                    const idMusic = results[0].id;
                    playlists.forEach(element => {
                        bot.dataBase.get("connection").exec('INSERT INTO ?? (`idTag`, `idMusic`) VALUES ( ?, ?)', ["musicsCorrelation", element, idMusic], (error, results, fields) => {
                            if (error) throw error;
                        });
                    });
                });
...
module.exports.run = async (bot) => {
    ...
    const job = new CronJob('0 30 05 * * *', async function () {
        bot.dataBase.get("connection").exec('SELECT `playListTag`, `playlist` FROM ??', ["playlistsList"], (error, results, fields) => {
            if (error) throw error;
